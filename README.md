...
<script>
  !(function (w, d) {
    ...
    s.async = true;
    s.src = 'https://cdn.pugs.dev/v0.1.1/pug.min.js';
...
<script
  async
  src="https://cdn.pugs.dev/v0.1.1/pug.min.js"
...
`persist: true` is recommended for consent-first flows. Once a visitor has actually made a choice, that recorded choice is what `init()` resolves — so if it is no longer `'granted'`, `init()` clears identity left over from an earlier consented visit. It leaves stored **identity** alone when the state came from your `initial` seed rather than from a stored choice, in either direction: adding a `'denied'` default to an existing site does not delete your existing visitors' identities, and a placeholder `'denied'` corrected by a later `optInTracking()` does not mint a new identity on every page load. (Queued events from a prior visit are still dropped unsent on any non-granted init, seed or not — see the [breaking-changes note](#unreleased--breaking-changes).)
...
- **Stored identifiers now expire** after `maxAgeDays` (default 365, absolute rather than sliding). Identity that used to live in `localStorage` forever now ages out, and the stored format changed — values written by an earlier version are unreadable, so they are treated as absent **and deleted** on first read — existing visitors get a fresh anonymous ID once, on upgrade, and any `identify()`ed external ID left by the old build leaves the device rather than sitting there under no deadline. The one exception is the consent record itself: a bare `granted`/`denied`/`cookieless` persisted by an earlier build is **adopted** as the user's recorded choice and re-persisted with a deadline, so a recorded opt-out survives the upgrade instead of silently reverting to your `initial` seed. On the CDN install, where each page pins its own `cdn.pugs.dev/vX.Y.Z` URL, roll the version out across all your pages together. The envelope rides `localStorage` as well as the cross-subdomain cookie, so pages on the old version and pages on the new one do not understand each other's stored format in *any* mode — including the default one, where same-origin pages thrash the anonymous ID back and forth. Worse for an `identify()`ed external ID: the old build has no envelope to unwrap, so it adopts the raw `1770000000000|you@example.com` as the profile's `distinctId` and sends it on every event until the upgrade is finished.
