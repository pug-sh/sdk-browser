...
const ACTIVITY_PERSIST_THROTTLE_MS = 10000;
 ...
 */
const mayWriteToDevice = () => isGrantedFn?.() ?? false;
export const configureSession = (projectId, sessionConfig, persistentStore, 
 ...
 * session.
 */
const armTabRegistry = ({ detectClosedTabs }) => {
        ...
        try {
            ...
            onPageHide = () => {
                try {
                    ...
                    const current = JSON.parse(tabsStorage.getItem(tabsKey) ?? '{}');
 ...
 */
const releaseTabRegistry = ({ purge }) => {
    if (onPageHide) {
    ...
    try {
        if (purge) {
            ...
            const tabs = JSON.parse(tabsStorage.getItem(tabsKey) ?? '{}');
            delete tabs[tabId];
...
export const rotate = () => {
    ...
    const next = { sessionId: uuidv7(), startTime: now, lastActivityTime: now, deviceId };
    state = next;
...
export const resetIdentity = () => {
    const now = Date.now();
    const next = { sessionId: uuidv7(), startTime: now, lastActivityTime: now, deviceId: uuidv7() };
    state = next;
