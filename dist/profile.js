...
let externalId = '';
let store = null;
export const configureProfile = (projectId, persistentStore, 
...
isGranted) => {
    ...
    if (stored) {
        ...
        if (stored.startsWith(RESERVED_DISTINCT_ID_PREFIX)) {
            log.warn(`Stored external ID uses the reserved "${RESERVED_DISTINCT_ID_PREFIX}" prefix, discarding it. The user will be treated as anonymous until identify() is called again.`);
            ...
            if (store && !store.removeItem(externalIdKey)) {
                log.error(`Failed to remove the reserved "${RESERVED_DISTINCT_ID_PREFIX}" external ID from storage; it will be re-read on the next page load.`);
            }
        }
        else {
            externalId = stored;
...
export const isIdentified = () => externalId !== '';
export const markIdentified = (id) => {
    externalId = id;
...
export const resolveDistinctId = () => {
