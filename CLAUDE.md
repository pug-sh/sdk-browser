...
Owns the consent state (`TrackingConsent`: `'granted' | 'denied'`). `createTrackingConsent({ projectId, defaultConsent, persist })` returns a controller. By default (`persist: false`) consent is in-memory only; callers that need consent across reloads should pass `defaultTrackingConsent` according to their own consent storage on each `init()`. When `persist: true` (via `InitOptions.persistTrackingConsent`), the controller reads/writes the decision to `localStorage` under the project-namespaced key `__pug_<projectId>_consent__` (`makeStorageKey(projectId, 'consent')`, guarded by `isStorageAvailable()`): on construction a stored `'granted'`/`'denied'` value takes precedence over `defaultConsent`, and `optIn()`/`optOut()` persist the new value — so an opt-out is sticky across reloads (the persist-opt-state-by-default behavior of posthog-js / mixpanel-js). The persisted decision is intentionally not cleared by `reset()` or `destroy()`, so a user's consent choice survives teardown/re-init. Storage faults are handled with privacy-biased asymmetry, because for a consent gate falling open (re-enabling tracking) is worse than falling closed: a **read** that throws fails safe to `'denied'` (never silently inherits a `'granted'` default for a user who may have opted out) and logs at `error`; a present-but-unparseable stored value is ignored with a `warn` (a `null`/absent value is the normal first run and stays quiet); a **write** that throws still flips the in-memory status (the current session is honored) and logs — at `error` for a failed `'denied'` persist (the opt-out won't survive the next reload), at `warn` for a failed `'granted'` persist. Note consent is read once at construction and is not synced across tabs (unlike `session.ts`, which re-reads storage on every call). The auto-capture controller stores the desired `autoCapture` selection and gates listener attachment on consent (read through an injected `isGranted` getter), so `setAutoCapture()` can be called before opt-in and the selection is re-applied on `optInTracking()`. When `init()` runs with consent denied it logs a one-time `log.warn`; integrators can detect the state via `isTrackingEnabled()`.
